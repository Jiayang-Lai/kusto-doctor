...
def create_table(client: KustoClient, database_name: str, table_name: str, schema: list[ColumnSchema]):
    """Creates a table in the specified database with the given schema."""
    logger.info(f"Attempting to create table {table_name}...")
    columns = ", ".join([f"{col.ColumnName} : {col.ColumnType.value}" for col in schema])
    create_table_cmd = f".create table {table_name} ({columns})"
    client.execute_mgmt(database_name, create_table_cmd)
    logger.info(f"Created table {table_name}!")


def check_if_table_exists(client: KustoClient, database_name: str, table_name: str) -> bool:
